...
public class AddressBook extends JFrame implements Runnable {
...
//      new AddressBook();
      ...
      Runnable myBook = new AddressBook();
      Thread myThread = new Thread(myBook);
   ...
   private class SearchAction extends AbstractAction {
      ...
      public void actionPerformed( ActionEvent e )
      {
         List<AddressBookEntry> peopleList;
         ...
         if ( lastName != null ) {
            ...
            //Each person with the same last name is stored in the List
            peopleList = database.findPerson( lastName );
            //Counts how many people have the same last name
            int peopleCount = peopleList.size();

            //Creates an array of AddressBookEntryFrame objects with the size of
            //how many people have the same lastname
            AddressBookEntryFrame [] frameArray = new AddressBookEntryFrame[ peopleCount ];

            int i = 0;

            //Check that the person searched is not null
            if ( peopleList != null ) {
               for ( AddressBookEntry frame : peopleList) {
              ...
              /* AddressBookEntryFrame entryFrame =
               ...
               entryFrame.setVisible( true );*/
                  ...
                  //Create window to display each person found
                  frameArray[i] = createAddressBookEntryFrame();
                  //Set the addressbook entry to display each person found
                  frameArray[i].setAddressBookEntry( frame );
                  //Display each window of information
